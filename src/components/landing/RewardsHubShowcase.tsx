...
const RewardsHubShowcase: React.FC = () => {
  ...
  const fallbackImageUrl = "https://placehold.co/400x300/FFC2E9/333333?text=Reward";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="md:flex md:items-center md:justify-between mb-12">
          <div className="md:w-1/2 mb-8 md:mb-0">
            <h2 className="text-3xl font-bold text-gray-800 mb-4 font-['Nunito',_sans-serif]">
              Rewards That Matter
            </h2>
          ...
          <div className="md:w-1/2 grid grid-cols-2 gap-4">
            {rewards.map((reward, index) => (
              <div key={index} className="bg-white/90 rounded-lg shadow-md overflow-hidden border border-[#FF85E2]">
                ...
                <div className="p-4">
                  <h3 className="font-semibold text-gray-800 font-['Nunito',_sans-serif]">{reward.name}</h3>
                  <div className="flex items-center mt-2">
                    <GoodCoinIcon className="w-5 h-5 mr-1 text-amber-500" />
                    <span className="text-amber-600 font-medium">{reward.coins}</span>
                  </div>
                </div>
              </div>
            ))}
